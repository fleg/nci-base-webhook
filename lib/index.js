...
BaseWebhook.prototype.register = function(app) {
	...
	// secret in url is optional because it can be provided in other way
	// at subclasses
	app.express.post(
		...
		function(req, res) {
			...
			if (project.webhooks[self.name].secret !== self.getSecret(req)) {
...
BaseWebhook.prototype.getSecret = function(req) {
	return req.params.secret;
};
